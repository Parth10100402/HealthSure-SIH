// HealthSure — Doctor Header Component (Fully Localized)
...
export const DoctorHeader: React.FC<DoctorHeaderProps> = ({ onToggleMobileDrawer }) => {
  ...
  const doctorId = user?.id || mockDoctorProfile.id;
  const speciality = mockDoctorProfile.speciality;
  const hospital = mockDoctorProfile.hospital;
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-[#072424]/95 backdrop-blur-md border-b border-[#DDE8E4] dark:border-[#1A3A3A] px-4 sm:px-6 py-3 transition-colors">
      <div className="flex items-center justify-between gap-4">
        {/* Left: Mobile hamburger & Doctor clinical badge */}
            ...
            {profileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-72 sm:w-80 max-w-[calc(100vw-24px)] bg-white dark:bg-[#0A2020] rounded-2xl border border-[#DDE8E4] dark:border-[#1A3A3A] shadow-2xl p-3 z-50 animate-in fade-in zoom-in-95 duration-150 space-y-2">
                <div className="p-3 rounded-xl bg-[#F5F9F7] dark:bg-[#0F2929] border border-[#DDE8E4]/60 dark:border-[#1A3A3A] space-y-1">
                  <div className="font-bold text-sm text-[#17324D] dark:text-[#E2EEF4] break-words">{doctorName}</div>
                  <div className="text-xs text-[#087F6D] dark:text-[#4FD1C5] font-semibold flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5 shrink-0" />
                    <span>{t.idLabel}: {doctorId}</span>
                  </div>
                  <div className="text-[11px] text-[#087F6D] dark:text-[#4FD1C5] font-semibold">{speciality} Specialist</div>
                  <div className="text-[11px] text-[#64748B] dark:text-[#7B9EA8] font-mono">{mockDoctorProfile.registrationNumber}</div>
                  <div className="text-[11px] text-[#64748B] dark:text-[#7B9EA8] pt-0.5 break-words">{hospital}</div>
                </div>
