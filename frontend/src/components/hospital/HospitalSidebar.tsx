// HealthSure — Hospital Staff Sidebar Component
// frontend/src/components/hospital/HospitalSidebar.tsx
...
export const HospitalSidebar: React.FC<HospitalSidebarProps> = ({ onCloseMobileDrawer }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {/* Navigation Items */}
        <nav className="p-3 space-y-1" aria-label="Hospital Menu">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
      ...
      {/* Bottom: Facility Status card & Logout */}
      <div className="p-3.5 border-t border-[#DDE8E4] dark:border-[#1A3A3A] space-y-2">
        <div className="p-3 rounded-xl bg-[#F5F9F7] dark:bg-[#0A2020] border border-[#DDE8E4] dark:border-[#1A3A3A] space-y-1">
          <div className="text-[10px] font-bold uppercase tracking-wider text-[#64748B] dark:text-[#7B9EA8]">
            Attached Rural Network
          </div>
          <div className="text-xs font-bold text-[#17324D] dark:text-[#E2EEF4] truncate">
            {mockHospitalProfile.assignedPHCs.length} Primary Health Centres
          </div>
          <div className="text-[11px] text-[#087F6D] dark:text-[#4FD1C5] font-semibold">
            Beds: {mockHospitalProfile.occupiedBeds}/{mockHospitalProfile.totalBeds} Occupied
          </div>
        </div>
        ...
        >
          ...
          <span>Sign Out Hospital Console</span>
        </button>
      </div>
    </aside>
  );
