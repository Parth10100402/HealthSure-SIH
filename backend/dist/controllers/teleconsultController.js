...
export const sessionPresenceStore = new Map();
/**
...
export function transitionTeleconsultationState(ctx) {
    ...
    if (to === 'ENDED' && !explicitUserAction && (currentStatus === 'LIVE' || currentStatus === 'CONNECTING')) {
        console.warn(`[${now}][${sessionId}][${role || 'SYSTEM'}][${requestId}] REJECTED_TRANSITION: Cannot transition ${currentStatus} -> ENDED without explicitUserAction! endpoint=${endpoint} reason=${reason}`);
        return false;
    }
    ...
    if (!allowed.includes(to)) {
        console.warn(`[${now}][${sessionId}][${role || 'SYSTEM'}][${requestId}] ILLEGAL_TRANSITION: ${currentStatus} -> ${to} is not allowed! endpoint=${endpoint} reason=${reason}`);
        return false;
    }
    ...
    if (tele) {
        tele.status = to;
        if (to === 'LIVE' && !tele.startedAt)
            tele.startedAt = new Date();
        if (to === 'ENDED')
            tele.endedAt = new Date();
    }
    console.log(`[${now}][${sessionId}][${role || 'SYSTEM'}][${requestId}] SESSION_STATE_TRANSITION: ${oldStatus} -> ${to} | endpoint=${endpoint} | caller=${actor} | reason=${reason} | explicit=${!!explicitUserAction}`);
    return true;
}
...
export const joinTeleconsult = async (req, res, next) => {
    try {
        ...
        let targetStatus = 'WAITING_FOR_DOCTOR';
...
export const liveTeleconsult = async (req, res, next) => {
    try {
        const rawId = String(req.params.id);
        const id = resolveCanonicalSessionId(rawId);
        const { role } = req.body;
...
async function persistSignalToDB(msg) {
    const prisma = getPrisma();
    if (!prisma)
        return;
    try {
...
async function readSignalsFromDB(sessionId, since) {
    const prisma = getPrisma();
    if (!prisma)
        return [];
    try {
...
export const getSignals = async (req, res, next) => {
    try {
        ...
        memoryMsgs
            ...
            .forEach((m) => { if (!mergedMap.has(m.id))
...
export const patchTeleconsult = async (req, res, next) => {
    try {
        ...
        if (networkMode)
            item.networkMode = networkMode;
        if (clinicalNotes)
            item.clinicalNotes = clinicalNotes;
