...
import type { HealthRecordEntity } from '../types/index.js';
...
export const getHealthRecords = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (recordType) {
      list = list.filter((r) => r.recordType === (recordType as string).toUpperCase());
    }
...
export const getHealthRecordById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
...
export const createHealthRecord = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const newRecord: HealthRecordEntity = {
      id: 'hr-' + Date.now(),
