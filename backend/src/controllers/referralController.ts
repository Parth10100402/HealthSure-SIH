...
import type { ReferralEntity } from '../types/index.js';

export const getReferrals = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (req.user?.role === 'PATIENT') {
      ...
      if (pid) list = list.filter((r) => r.patientId === pid);
      ...
      if (dh) list = list.filter((r) => r.receivingHospitalId === dh.id);
    }

    if (status) list = list.filter((r) => r.status === (status as string).toUpperCase());
    if (priority) list = list.filter((r) => r.priority === (priority as string).toUpperCase());
    if (patientId) list = list.filter((r) => r.patientId === patientId);
    if (hospitalId) list = list.filter((r) => r.receivingHospitalId === hospitalId);
    if (phcId) list = list.filter((r) => r.referringFacilityId === phcId);
...
export const getReferralById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
...
export const createReferral = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const body = createReferralSchema.parse(req.body);

    const newRef: ReferralEntity = {
      id: 'ref-' + Date.now(),
...
export const patchReferral = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (body.tokenNumber) ref.tokenNumber = body.tokenNumber;
